...
class CoreAttention(MegatronModule):
    ...
    def forward(self, query_layer, key_layer,
        ...
        matmul_input_buffer = matmul_input_buffer.float()
        query_layer = query_layer.float()/self.norm_factor
        ...
        attention_probs = attention_probs.view(output_size[0] * output_size[1],
                                               output_size[2], -1)
        attention_probs = attention_probs.to(value_layer)
...
class FlashSelfAttention(torch.nn.Module):
    ...
    def __init__(self, causal=False, softmax_scale=None, attention_dropout=0.0,
        ...
        super().__init__()
        args = get_args()
        self.window_size = args.localsize
...
class ParallelAttention(MegatronModule):
    ...
    def forward(self, hidden_states, attention_mask,
        ...
        if self.attention_type == AttnType.self_attn:
            ...
            query_layer = query_layer.view(query_layer.shape[0], query_layer.shape[1], self.num_attention_heads_per_partition, self.hidden_size_per_attention_head)
            key_layer = key_layer.view(key_layer.shape[0], key_layer.shape[1], self.num_attention_heads_per_partition, self.hidden_size_per_attention_head)
            value_layer = value_layer.view(value_layer.shape[0], value_layer.shape[1], self.num_attention_heads_per_partition, self.hidden_size_per_attention_head)
        else:
...
class ParallelTransformerLayer(MegatronModule):
    ...
    def forward(self, hidden_states, attention_mask=None,
        ...
        # # NOTE(xzdong2): we only need the first element(the real_hidden_states) when using Megatron's checkpointing
        # # e.p: hidden_states: (real_hidden_states, moe_loss)
