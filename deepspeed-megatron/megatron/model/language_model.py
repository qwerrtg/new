...
class TransformerLanguageModel(MegatronModule):
    ...
    def state_dict_for_save_checkpoint(self, prefix='', keep_vars=False):
        ...
        if self.add_encoder:
            ...
            if args.apply_lora:
                from megatron.core.tensor_parallel.lora_layer import lora_state_dict
                encoder_state_dict = lora_state_dict(encoder_state_dict, self.encoder, args.merge_weights)
        ...
        if args.apply_lora and not args.merge_weights:
            pop_keys = [k for k in state_dict_.keys() if k!=self._encoder_key]
            for key in pop_keys:
                state_dict_.pop(key)
        return state_dict_
