...
def train(forward_step_func, model, optimizer, opt_param_scheduler,
    ...
    if train_data_iterator is not None:
        ...
        valid_samples = len(valid_data_iterator)
        samples_flags = get_accelerator().LongTensor(
            [int(args.train_samples), int(valid_samples)])
    else:
        samples_flags = get_accelerator().LongTensor([0, 0])
    # Broadcast samples_flags.
    torch.distributed.broadcast(samples_flags,
                                mpu.get_tensor_model_parallel_src_rank(),
                                group=mpu.get_tensor_model_parallel_group())
    args.train_samples = samples_flags[0].item()
    valid_samples = samples_flags[1].item()
    args.train_iters = args.train_samples // args.global_batch_size
    args.eval_iters = valid_samples // args.global_batch_size
    print_rank_0('resetting training iterations to {}'.format(args.train_iters))
    consumed_samples_once = mpu.get_data_parallel_world_size() * args.micro_batch_size * get_num_microbatches()
    while iteration < args.train_iters and (args.train_tokens is None or \
        args.consumed_train_tokens < args.train_tokens) and args.consumed_train_samples <= args.train_samples-consumed_samples_once:
...
def evaluate(forward_step_func,
    ...
    total_loss_dict = {}
    valid_samples = args.eval_iters * args.global_batch_size
    consumed_samples_once = mpu.get_data_parallel_world_size() * args.micro_batch_size * get_num_microbatches()
    with torch.no_grad():
        iteration = 0
        while iteration < args.eval_iters and args.consumed_valid_samples <= valid_samples-consumed_samples_once:
            iteration += 1
    ...
    if iteration == 0:
        print_rank_0("Vaild data is not enough! Skip evalute.")
