...
def _get_blockmask(seqlen, local_size=256, blocksize_q=16, blocksize_k=128):
    assert (
...
def _get_fullmask(
    ...
    layout = _get_blockmask(max_seq_length, local_size, blocksize_q, blocksize_k)
    qlen, klen = layout.shape
    seqlen = qlen * blocksize_q
    assert seqlen == blocksize_k * klen
    fullmask = layout.view(qlen, 1, klen, 1).repeat(
        1, blocksize_q, 1, blocksize_k
    )
...
def get_ltor_masks_and_position_ids(data,
    ...
    if args.localsize is not None:
        attention_mask = _get_fullmask(args.seq_length, args.localsize).to(attention_mask)
