...
def generate_tokens_probs_and_return_on_first_stage(
    ...
    num_repeat_penalty = args.num_repeat_penalty
    no_repeat_logits_processor = None
    if repeat_penalty > 0 or num_repeat_penalty > 0:
        no_repeat_logits_processor = RepetitionPenaltyLogitsProcessor(
            repeat_penalty, num_repeat_penalty
        )
    min_length_logits_processor = None
    min_len = args.min_len
    if min_len > 0:
        min_length_logits_processor = MinLengthLogitsProcessorRightPad(
            min_len, termination_id
        )
    ...
    with torch.no_grad():
        ...
        for context_length in range(min_prompt_length, max_sequence_length):
            ...
            if mpu.is_pipeline_last_stage():
                ...
                if no_repeat_logits_processor is not None:
                    next_token_logits = no_repeat_logits_processor(
                        tokens[:, :context_length], next_token_logits, input_lens
                    )
                if min_length_logits_processor is not None:
                    next_token_logits = min_length_logits_processor(
                        tokens[:, :context_length], next_token_logits, input_lens
                    )
                # Always the last stage should have an output.
                assert logits is not None
