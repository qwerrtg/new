...
###############################################################################
##以下参数需更换为本地路径
from_pretrained="/data/ckpt/ckpt-tp8"
data_path="/data/dataset/sft/seq_length_32768_text_document"
vocab_path="/data/tokenizer/tokenizer"
output_home="/data/sft-lora-output"
lora_from_pretrained="/data/sft-lora-output/checkpoint/" #需要更改为lora权重保存的文件夹位置
...
megatron_options=" \
    ...
    --lora-from-pretrained=${lora_from_pretrained} \
...
# --hostfile=hostfile.txt --master_addr 172.19.16.131 --master_port 49999 --include localhost:0
ds  ${dir}/../../sft_iFlytekSpark_lora_merge.py ${megatron_options} ${data_options} ${deepspeed_options} &>> ${log_path}/${jobname}_${host}_${current_time}.log
